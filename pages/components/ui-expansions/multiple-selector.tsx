    ...
    return (
      ...
      >
        ...
        {open && <div className="relative">
          <CommandList
            className={cn('absolute mt-2 top-0 z-10 w-full rounded-md bg-popover text-popover-foreground outline-none animate-in', creatable && inputValue?.length ? 'border shadow-sm' : undefined)}>
            {isLoading ? (
              <>{loadingIndicator}</>
            ) : (
              <>
