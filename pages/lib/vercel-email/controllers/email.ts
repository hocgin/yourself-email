...
class Email {
   ...
   * @param headers
   */
  static async send(email: IEmail, headers?: Record<string, string>) {
    // convert email to IMCEmail (MailChannels Email)
    const mcEmail: IMCEmail = Email.convertEmail(email, headers);
   ...
   * @protected
   */
  protected static convertEmail(email: IEmail, headers?: Record<string, string>): IMCEmail {
